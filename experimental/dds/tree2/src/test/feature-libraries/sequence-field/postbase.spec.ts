...
function postbase(
	...
	return postbaseI(change, tagChange(base, baseRev ?? tag1));
}

// Rebasing should not change the content of a Revive
const postbaseRepair = createFakeRepair((_revision, _index) => "fakeData");

describe("SequenceField - Postbase", () => {
	describe("no changes ↷ *", () => {
		for (const [name, testCase] of Object.entries(cases)) {
			it(`no changes ↷ ${name}`, () => {
				const actual = postbase([], testCase);
	...
	describe("* ↷ no changes", () => {
		for (const [name, testCase] of Object.entries(cases)) {
			it(`${name} ↷ no changes`, () => {
				const actual = postbase(testCase, cases.no_change);
	...
	it("modify ↷ modify", () => {
		...
		const actual = postbase(change2, change1);
		assert.deepEqual(actual, expected);
	});

	it("insert ↷ modify", () => {
		const actual = postbase(cases.insert, cases.modify);
		assert.deepEqual(actual, cases.insert);
	});

	it("modify insert ↷ modify", () => {
		const actual = postbase(cases.modify_insert, cases.modify);
		assert.deepEqual(actual, cases.modify_insert);
	});

	it("delete ↷ modify", () => {
		const actual = postbase(cases.delete, cases.modify);
		assert.deepEqual(actual, cases.delete);
	});

	it("revive ↷ modify", () => {
		const revive = composeAnonChanges([
			Change.revive(0, 2, { revision: tag1, localId: brand(0) }, postbaseRepair),
			Change.revive(4, 2, { revision: tag1, localId: brand(2) }, postbaseRepair),
			Change.revive(10, 2, { revision: tag1, localId: brand(4) }, postbaseRepair),
		]);
		...
		const actual = postbase(revive, mods);
	...
	it("modify ↷ delete", () => {
		...
		const actual = postbase(mods, deletion);
	...
	it("insert ↷ delete", () => {
		...
		const deletion = Change.delete(1, 3);
		const actual = postbase(insert, deletion);
		const expected = composeAnonChanges([
	...
	it("revive ↷ delete", () => {
		const revive = composeAnonChanges([
			Change.revive(0, 1, { revision: tag1, localId: brand(0) }, postbaseRepair),
			Change.revive(3, 1, { revision: tag1, localId: brand(1) }, postbaseRepair),
			Change.revive(8, 1, { revision: tag1, localId: brand(2) }, postbaseRepair),
		]);
		const deletion = Change.delete(1, 3);
		const actual = postbase(revive, deletion, tag2);
		const expected = composeAnonChanges([
			...
			Change.revive(5, 1, { revision: tag1, localId: brand(2) }, postbaseRepair),
		]);
		assert.deepEqual(actual, expected);
	});
	...
	it("redundant revive ↷ related delete", () => {
		...
		const expected = composeAnonChanges([
			...
			Change.redundantRevive(0, 1, { revision: tag1, localId: brand(1) }, postbaseRepair),
			// Overlapping revive is no longer redundant
			Change.revive(1, 1, { revision: tag1, localId: brand(1) }, postbaseRepair, {
				revision: tag2,
				localId: brand(0),
				adjacentCells: [{ id: brand(0), count: 1 }],
			}),
			// Later revive is unaffected
			Change.redundantRevive(1, 1, { revision: tag1, localId: brand(3) }, postbaseRepair),
		]);
		assert.deepEqual(actual, expected);
	});
	...
	it("redundant revive ↷ unrelated delete", () => {
		...
		const deletion = Change.delete(1, 1);
		const actual = postbase(revive, deletion, tag3);
		const expected = [
	...
	it("blocked revive ↷ revive", () => {
		...
		const actual = postbase(revive1, revive2, tag2);
		const expected = [
			Mark.revive(
				fakeRepair(tag1, 0, 1),
				{ revision: tag2, localId: brand(1) },
	...
	it("delete ↷ overlapping delete", () => {
		...
		const actual = postbase(deleteA, deleteB, tag1);
		// Deletes --E-G
		const expected = [
			{ count: 2 },
	...
	it("delete ↷ earlier delete", () => {
		...
		const actual = postbase(deleteA, deleteB);
		// Deletes -DE
	...
	it("move ↷ overlapping delete", () => {
		...
		const actual = postbase(move, deletion, tag1);
	...
	it("modify ↷ insert", () => {
		...
		const actual = postbase(mods, insert);
		assert.deepEqual(actual, expected);
	});
	...
	it("delete ↷ insert", () => {
		...
		const actual = postbase(deletion, insert);
		assert.deepEqual(actual, expected);
	});
	...
	it("insert ↷ insert", () => {
		...
		const actual = postbase(insertA, insertB);
	...
	it("revive ↷ insert", () => {
		const revive = composeAnonChanges([
			Change.revive(0, 1, { revision: tag1, localId: brand(0) }, postbaseRepair),
			Change.revive(3, 2, { revision: tag1, localId: brand(1) }, postbaseRepair),
			Change.revive(7, 1, { revision: tag1, localId: brand(3) }, postbaseRepair),
		]);
		// TODO: test both tiebreak policies
		const insert = Change.insert(2, 1);
		const actual = postbase(revive, insert);
		const expected = composeAnonChanges([
			Change.revive(0, 1, { revision: tag1, localId: brand(0) }, postbaseRepair),
			Change.revive(4, 2, { revision: tag1, localId: brand(1) }, postbaseRepair),
			Change.revive(8, 1, { revision: tag1, localId: brand(3) }, postbaseRepair),
		]);
		assert.deepEqual(actual, expected);
	});
	...
	it("redundant revive ↷ insert", () => {
		...
		const actual = postbase(revive, insert);
		const expected = composeAnonChanges([
			Change.redundantRevive(0, 1, { revision: tag1, localId: brand(0) }, postbaseRepair),
			Change.redundantRevive(2, 2, { revision: tag1, localId: brand(1) }, postbaseRepair),
		]);
		assert.deepEqual(actual, expected);
	});
	...
	it("modify ↷ revive", () => {
		...
		const revive = Change.revive(2, 1, { revision: tag1, localId: brand(0) }, postbaseRepair);
		const expected = composeAnonChanges([
			// Modify at earlier index is unaffected
			Change.modify(0, TestChange.mint([0], 1)),
			// Modify at later index has its index increased
		...
		const actual = postbase(mods, revive);
		assert.deepEqual(actual, expected);
	});
	...
	it("delete ↷ revive", () => {
		...
		const revive = Change.revive(3, 1, { revision: tag1, localId: brand(0) }, postbaseRepair);
		const expected = composeAnonChanges([
			// Delete with earlier index is unaffected
			Change.delete(0, 1, brand(0)),
			// Delete at overlapping index is split
		...
		const actual = postbase(deletion, revive);
		assert.deepEqual(actual, expected);
	});

	it("insert ↷ revive", () => {
		const insert = composeAnonChanges([Change.insert(0, 1, 1), Change.insert(3, 1, 2)]);
		const revive = Change.revive(1, 1, { revision: tag1, localId: brand(0) }, postbaseRepair);
		const actual = postbase(insert, revive);
		const expected = composeAnonChanges([Change.insert(0, 1, 1), Change.insert(4, 1, 2)]);
		assert.deepEqual(actual, expected);
	});
	...
	it("reviveAA ↷ reviveB => BAA", () => {
		...
		const reviveB = Change.revive(0, 1, { revision: tag2, localId: brand(0) }, postbaseRepair);
		...
		const actual = postbase(reviveAA, reviveB);
		assert.deepEqual(actual, expected);
	});
	...
	it("reviveBB ↷ reviveA => BBA", () => {
		const reviveBB = Change.revive(0, 2, { revision: tag2, localId: brand(0) }, postbaseRepair);
		...
		const expected = Change.revive(0, 2, { revision: tag2, localId: brand(0) }, postbaseRepair);
		const actual = postbase(reviveBB, reviveA);
		assert.deepEqual(actual, expected);
	});

	it("reviveBB ↷ reviveA => ABB", () => {
		const reviveBB = Change.revive(5, 2, { revision: tag2, localId: brand(0) }, postbaseRepair);
		const reviveA = Change.revive(
			5,
			1,
			{
		...
		const expected = Change.revive(6, 2, { revision: tag2, localId: brand(0) }, postbaseRepair);
		const actual = postbase(reviveBB, reviveA);
		assert.deepEqual(actual, expected);
	});
	...
	it("reviveA ↷ reviveBB => BAB", () => {
		...
		const reviveBB = Change.revive(5, 2, { revision: tag2, localId: brand(5) }, postbaseRepair);
		const expected = Change.revive(
			6,
			1,
			{ revision: tag1, localId: brand(6), lineage },
		...
		const actual = postbase(reviveA, reviveBB);
		assert.deepEqual(actual, expected);
	});
	...
	it("reviveAA ↷ reviveCB => CBAA", () => {
		...
		const reviveB = composeAnonChanges([
			Change.revive(0, 1, { revision: tag2, localId: brand(0) }, postbaseRepair),
			Change.revive(0, 1, { revision: tag3, localId: brand(0) }, postbaseRepair),
		]);
		const expected = Change.revive(
			2,
			2,
	...
	it("intentional revive ↷ same revive", () => {
		...
		const reviveB = Change.revive(0, 1, { revision: tag1, localId: brand(2) }, postbaseRepair);
		const actual = postbase(reviveA, reviveB, tag2);
		const expected = composeAnonChanges([
			Change.intentionalRevive(0, 1, { revision: tag1, localId: brand(1) }, postbaseRepair),
			Change.redundantRevive(
				1,
				1,
			...
			Change.intentionalRevive(2, 1, { revision: tag1, localId: brand(3) }, postbaseRepair),
		]);
		assert.deepEqual(actual, expected);
	});

	it("revive ↷ same revive (base within curr)", () => {
		const reviveA = Change.revive(0, 3, { revision: tag1, localId: brand(1) }, postbaseRepair);
		const reviveB = Change.revive(0, 1, { revision: tag1, localId: brand(2) }, postbaseRepair);
		const actual = postbase(reviveA, reviveB, tag2);
		const expected = composeAnonChanges([
			Change.revive(0, 1, { revision: tag1, localId: brand(1) }, postbaseRepair),
			Change.redundantRevive(1, 1, { revision: tag1, localId: brand(2) }, postbaseRepair),
			Change.revive(2, 1, { revision: tag1, localId: brand(3) }, postbaseRepair),
		]);
		assert.deepEqual(actual, expected);
	});

	it("revive ↷ same revive (curr within base)", () => {
		const reviveA = Change.revive(0, 1, { revision: tag1, localId: brand(2) }, postbaseRepair);
		const reviveB = Change.revive(0, 3, { revision: tag1, localId: brand(1) }, postbaseRepair);
		const actual = postbase(reviveA, reviveB, tag2);
	...
	it("concurrent inserts ↷ delete", () => {
		...
		const insertB2 = postbaseTagged(insertB, delA);
		const insertC2 = postbaseTagged(insertC, delA, insertB2);
	...
	it("concurrent inserts ↷ connected delete", () => {
		...
		const insertD2 = postbaseTagged(insertD, delA, delB, delC);
		const insertE2 = postbaseTagged(insertE, delA, delB, delC, insertD2);
		const expected = Change.insert(1, 1);
	...
	it("concurrent insert and move ↷ delete", () => {
		...
		const insertB2 = postbaseTagged(insertB, delA);
		const moveC2 = postbaseTagged(moveC, delA, insertB2);
	...
	it("modify ↷ move", () => {
		...
		const postbased = postbase(modify, move);
		assert.deepEqual(postbased, expected);
	});
	...
	it("delete ↷ move", () => {
		...
		const expected = Change.delete(1, 2);
		const postbased = postbase(deletion, move);
		assert.deepEqual(postbased, expected);
	});
	...
	it("move ↷ move", () => {
		...
		const postbased = postbase(moveB, moveA);
		assert.deepEqual(postbased, expected);
	});
});
