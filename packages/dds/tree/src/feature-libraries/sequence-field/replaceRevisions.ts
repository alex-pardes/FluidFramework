...
function replaceEffectRevisions<TMark extends MarkEffect>(
	...
	switch (type) {
		case NoopMarkType:
			return mark;
		case "Insert":
			return replaceRevision<TMark & HasRevisionTag>(mark, replacer);
...
function replaceDetachRevisions<TDetach extends Detach>(
	...
	const updated = replaceRevision(detach, replacer) as Mutable<TDetach>;
...
function replaceRevision<T extends HasRevisionTag>(input: T, replacer: RevisionReplacer): T {
